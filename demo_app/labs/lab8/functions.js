const experimentFunctions = {
    ...
    forceSetStep(stepNumber) {
        ...
        const instructions = [
            ...
            "Нажмите кнопку 'Включить' и дождитесь закипания воды (t₂ = 100°C).",
            "Дайте воде покипеть некоторое время, затем нажмите 'Выключить'.",
        ...
        switch(stepNumber) {
            ...
            case 4:
                ...
                if (!wasHeaterOn) {
                    document.getElementById('heater-toggle').textContent = 'Включить';
    ...
    toggleHeater() {
        ...
        if (heaterToggleBtn) {
            heaterToggleBtn.textContent = state.heaterOn ? 'Выключить' : 'Включить';
    ...
    updateTimer(isBoiling = false) {
        ...
        if (!isBoiling) {
            ...
            if (state.elapsedTime >= HEAT_TIME && !state.boiling) {
                ...
                if (wasHeaterOn) {
                    setTimeout(() => {
                        ...
                        const heaterToggleBtn = document.getElementById('heater-toggle');
                        if (heaterToggleBtn) {
                            heaterToggleBtn.textContent = 'Выключить';
            ...
            if (state.heaterOn && state.elapsedTime >= 40) {
                ...
                const heaterToggleBtn = document.getElementById('heater-toggle');
                if (heaterToggleBtn) {
                    heaterToggleBtn.textContent = 'Включить';
